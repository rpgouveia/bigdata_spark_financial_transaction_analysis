...
public class TopCategoriesByState {
    ...
    public static void main(String[] args) {
        ...
        SparkConf conf = new SparkConf()
                .setAppName("TopCategoriesByState")
                .setMaster("local[*]");
        ...
        System.out.println("Iniciando TopCategoriesByState com Spark RDDs\n");
        ...
        JavaRDD<String> filteredData = data.filter(line -> {
            try {
                ...
                // Lógica principal: ACEITA apenas se for um estado dos EUA
                return VALID_US_STATES.contains(location);
        ...
        // Passo 1: Mapeia para ((estado, mcc), 1) e conta ocorrências
        JavaPairRDD<Tuple2<String, String>, Long> stateMccCounts = filteredData
                .mapToPair(line -> {
                    String[] fields = line.split(",");
                    String state = fields[8].trim().toUpperCase();  // merchant_state
                    String mcc = fields[10].trim();  // mcc code
                    return new Tuple2<>(new Tuple2<>(state, mcc), 1L);
                })
                .reduceByKey((a, b) -> a + b);

        // Passo 2: Transforma para (estado, CategoryCount)
        JavaPairRDD<String, CategoryCount> stateCategories = stateMccCounts
                .mapToPair(tuple -> {
                    String state = tuple._1._1;
                    String mcc = tuple._1._2;
                    long count = tuple._2;
                    return new Tuple2<>(state, new CategoryCount(mcc, count));
                });

        // Passo 3: Agrupa por estado e pega top 3
        JavaPairRDD<String, String> topCategoriesByState = stateCategories
                .groupByKey()
        ...
        // Passo 4: Calcula total de transações por estado para ordenação
        JavaPairRDD<String, Long> stateTotalCounts = stateMccCounts
                .mapToPair(tuple -> new Tuple2<>(tuple._1._1, tuple._2))
                .reduceByKey((a, b) -> a + b);

        // Passo 5: Join para adicionar total e ordenar
        JavaPairRDD<String, Tuple2<String, Long>> sortedResults = topCategoriesByState
                .join(stateTotalCounts)  // Resultado: (estado, (categories, total))
                .mapToPair(tuple -> new Tuple2<>(tuple._2._2, new Tuple2<>(tuple._1, tuple._2._1)))  // (total, (estado, categories))
                .sortByKey(false)  // Ordena por total decrescente
                .mapToPair(tuple -> new Tuple2<>(tuple._2._1, new Tuple2<>(tuple._2._2, tuple._1)));  // (estado, (categories, total))
        ...
        sortedResults.take(20).forEach(tuple -> {
            ...
            System.out.println(state + " (Total: " + total + " transações)");
            System.out.println("  " + categories);
        });

        // Calcula estatísticas globais
        long totalStates = topCategoriesByState.count();
        long totalUniqueMCCs = stateMccCounts
